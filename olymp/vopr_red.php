<?php 
	...
	$q=$_SESSION['KodVopr'];
	$strSQL = "SELECT * FROM Vopr where KodVopr='$q'";
...
<script>	
	...
	function Sel_Change()
	{
		...
		xmlhttp.open('POST', 'vopr_save.php', true); // Открываем асинхронное соединение
...
<body class="bd">
	...
	<div id="centeredmenu">
		<ul>
			...
			<li><a href="prepod_olymp_insert.php" class="active">Редактирование вопроса номер <?=$row['Nom']?></a></li>
	...
	<div class="forma_vvoda2">
		<form align="left">
			<table class="vopros_otvet">
				<tr>
					<td>
						<p class="text_near_knopka" >Вопрос:</p><textarea class="pole_vvoda2" id="vopr1" name="vopr1" rows="10" cols="45"  ><?=$row['Vopr']?></textarea>
					</td>
					<td>
						<p class="text_near_knopka" class="text_near_knopka">Правильный ответ:</p><textarea class="pole_vvoda2" id="otv1" name="otv1" rows="10" cols="45" ><?=$row['Otv']?></textarea>
					</td>
				</tr>
				<tr>
					<td>
						<p class="text_near_knopka" ><br>Макс.балл за вопрос: <input class="pole_vvoda3" id="ocenka1" type="text" name="ocenka1" value=<?=$row['Ocenka']?> /></p>
					</td>
				</tr>
			</table>			
			<table align="center">
				<tr>
					<td>
						<input class="knopka" type="button" name="b1" value="Сохранить" onclick="b1_click()" />
					</td>
					<td>
