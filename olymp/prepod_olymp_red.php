<?php 
	...
	$q=$_SESSION['KodOlymp'];
	$strSQL = "SELECT * FROM Vopr where KodOlympF='$q' ";
...
<script>
	function del()
	{
		select = document.getElementById("cur_vopr"); // Выбираем  select по id
		...
		xmlhttp.open('POST', 'vopr_del.php', true); // Открываем асинхронное соединение
		xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded'); // Отправляем кодировку
		var str='xKodVopr='+encodeURIComponent(value);
	...
	function perehod()
	{
		...
		//alert("Value: " + value + "\nТекст: " + text); // Вывод алерта для проверки значений
		var xmlhttp = getXmlHttp(); // Создаём объект XMLHTTP
		xmlhttp.open('POST', 'change2.php', true); // Открываем асинхронное соединение
		xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded'); // Отправляем кодировку
		var str='xKodVopr='+encodeURIComponent(value);
		xmlhttp.send(str); // Отправляем POST-запрос
		xmlhttp.onreadystatechange = 
		...
		document.location.href = 'vopr_red.php';
	}
	function getXmlHttp() 
	{
		var xmlhttp;
...
<body class="bd">
	...
	<div id="centeredmenu">
		<ul>
			...
			<li><a href="prepod_olymp.php">Назад</a></li>
			<li><a href="prepod_olymp_red.php" class="active">Редактирование олимпиады "<?=$rw?>"</a></li>
	...
	{
	?>
		<div class="text_near_knopka">Текущий вопрос:
		<select class="combobox" id="cur_vopr" type=text name="cur_vopr">  </div>
			<?php  	
			while ($row1 = mysql_fetch_array($rs)): 
			?> 
				<option value=<?=$row1['KodVopr']?> ><p><?=$row1['Nom']?></p></option>
			<? endwhile ?> 	
		</select>
		<br></br>
		<div id="div2" align="center"> 
			<input class="knopka" id="b_insert" type="button" value="Добавить новый вопрос" onclick="location.href='olymp_vopr_insert.php'" />
			<input class="knopka" id="b_change" type="button" value="Редактировать вопрос" onclick="perehod()" />
			<input class="knopka" id="b_delete" type="button" value="Удалить вопрос" onclick="del()" /> 
			<input class="knopka" id="b_delete" type="button" value="Изменить информацию об олимпиаде" onclick="location.href='olymp_info_red.php'" /> 
		</div>
		<div id="div1">
			<table class="StandardTable" align="center" border="1"> 
				...
				<tbody>
					...
					?> 
						<tr> 
							<td class="pocentru"><p><?=$row2['Nom']?></p></td> 
							<td><p><?=$row2['Vopr']?></p></td> 
							<td><p><?=$row2['Otv']?></p></td>	
							<td class="pocentru"><p><?=$row2['Ocenka']?></p></td>						
						</tr>
					<? endwhile ?> 
				</tbody>
			</table>
	...
	{
	?>	
		<div class="text_near_knopka">Пока олимпиада не содержит ни одного вопроса!</div>
		<div id="div2" align="center"> 
			<input class="knopka" id="b_insert" type="button" value="Добавить новый вопрос" onclick="location.href='olymp_vopr_insert.php'" />
		</div>	
	<?php
